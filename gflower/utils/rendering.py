...
from gflower.datasets.d4rl import load_environment
from gflower.utils.debug import GetMapIndex
...
class MazeRenderer:
    ...
    def set_obstacle(self, obs_center, obs_radius):
        self.obs_num = len(obs_center)
        for i in range(self.obs_num):
            obs_center[i] = np.array(obs_center[i]) + .5
            obs_radius[i] = np.array(obs_radius[i])

        if len(self.bounds) == 4:
            _, iscale, _, jscale = self.bounds
            for i in range(self.obs_num):
                obs_center[i][0] /= iscale
                obs_radius[i][0] /= iscale
                obs_center[i][1] /= jscale
                obs_radius[i][1] /= jscale
        else:
            raise RuntimeError(f'Unrecognized bounds for {self.env_name}: {self.bounds}')
    ...
    def renders(self, observations, conditions=None, title=None):
        ...
        plt.imshow(self._background.transpose() * .5,
            extent=self._extent, 
            origin='lower', 
        ...
        if self.b_obs:
            for i in range(self.obs_num):
                ...
                if self.nMap == 1:
                    x = self.obs_radius[i][0]*np.sqrt(np.abs(np.cos(theta)))*np.sign(np.cos(theta)) + self.obs_center[i][0]
                    y = self.obs_radius[i][1]*np.sqrt(np.abs(np.sin(theta)))*np.sign(np.sin(theta)) + self.obs_center[i][1]
                else:
                    x = self.obs_radius[i][0]*np.cos(theta) + self.obs_center[i][0]
                    y = self.obs_radius[i][1]*np.sin(theta) + self.obs_center[i][1]
    ...
    def composite(self, savepath, paths, ncol=5, **kwargs):
        '''
            ...
            obs_center: List[(center_x1, center_y1), (center_x2, center_y2), ...]
            obs_radius: List[(r_x1, r_y1), (r_x2, r_y2), ...]
        '''
...
class Maze2dRenderer(MazeRenderer):
    ...
    def __init__(self, env, observation_dim=None):
        ...
        self._extent = (0, 1, 0, 1)
        self.b_obs = False
        self.bounds = MAZE_BOUNDS[self.env_name]
