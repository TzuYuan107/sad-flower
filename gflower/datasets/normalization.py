...
class DatasetNormalizer:
    ...
    def normalize_scaling(self, x, key):
        return self.normalizers[key].normalize_scaling(x)
...
def flatten(dataset, path_lengths):
    '''
        flattens dataset of { key: [ n_episodes x max_path_lenth x dim ], dim for channel }
...
class Normalizer:
    ...
    def normalize_scaling(self, *args, **kwargs):
        raise NotImplementedError()
...
class DebugNormalizer(Normalizer):
    ...
    def normalize_scaling(self, x, *args, **kwargs):
        return x
...
class LimitsNormalizer(Normalizer):
    ...
    def normalize_scaling(self, x):
        ## scale from [ 0, 1 ]
        x = x / (self.maxs - self.mins)
        ## scale from [ -1, 1 ]
        x = 2 * x
        return x
